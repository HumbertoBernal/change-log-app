...
export default withApiAuthRequired(function handler(
  ...
  if (typeof logId !== "string") {
    ...
    try {
      if (method === "DELETE") {
        MongoService.deleteLog(logId).then((result) => {
        ...
        logId.length === 24
          ? MongoService.getLog(logId).then((log) => {
              log
        ...
        MongoService.putLog(logId, data).then((result) => {
          result
        ...
        MongoService.patchLog(logId, data).then((result) => {
