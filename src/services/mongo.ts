...
import { CreateLogAPI, CreateLog, FilterLog, Log, LogQuery, PatchLog } from "@/modules/logs/types";
...
const list = async <TData>(query: any, page: number, dbName: string, collectionName: string) => {
  await client.connect();
  ...
  const findResult = (await collection
    ...
    .toArray()) as unknown as TData[];
  client.close();

  return { count, findResult };
};

const update = async <TData>(
  id: string,
  project: TData,
  dbName: string,
...
const createProject = async ({ name, description, created_by }: CreateProjectAPI) =>
  await create<CreateProjectAPI>({ name, description, created_by }, dbName, projectCollectionName);
const listProjects = async (page: number, filters?: FilterProject) =>
  await list<Project>(getProjectQuery(filters), page, dbName, projectCollectionName);
const getProject = async (id: string) => await get<Project>(id, dbName, projectCollectionName);
const patchProject = async (id: string, project: PatchProject) =>
  await update<PatchProject>(id, project, dbName, projectCollectionName);
const putProject = async (id: string, project: Project) =>
  await update<Project>(id, project, dbName, projectCollectionName);
const deleteProject = async (id: string) => await remove(id, dbName, projectCollectionName);

const createLog = async (log: CreateLogAPI) =>
  await create<CreateLogAPI>({ ...log }, dbName, logCollectionName);
const listLogs = async (page: number, filters?: FilterLog) =>
  await list<Log>(getLogQuery(filters), page, dbName, logCollectionName);
const getLog = async (id: string) => await get<Log>(id, dbName, logCollectionName);
const patchLog = async (id: string, log: PatchLog) =>
  await update<PatchLog>(id, log, dbName, logCollectionName);
const putLog = async (id: string, log: Log) =>
  await update<Log>(id, log, dbName, logCollectionName);
